    ...
    public class WinDectector : IWinDetector
    {
        public Piece GetWinner(IGrid grid)
        {
            foreach (var p in new[] { Piece.X, Piece.O })
                if (GetWinnableLines(grid.Size).Any(line => line.All(pos => p == grid[pos])))
                    return p;
            return null;
        }
        ...
        {
            var ztgs = ZeroToGridSize(gridSize);
            ...
            results.Add(ztgs.Select(diag => new Point(diag)).ToArray());
            results.Add(ztgs.Select(diag => new Point(diag, gridSize - diag)).ToArray());
            return results;
        }
